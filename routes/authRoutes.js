...
const { isNotLoggedIn } = require("../util/isNotLoggedIn");
  ...
  (req, res, next) => {
    passport.authenticate("json", (authError, user, info) => {
      ...
      if (!user) {
        ...
        return next(
          new customError("not validate id, pwd", 404, "wrong id or pwd")
        );
      }

      // done(null, exUser)가 처리된경우, 즉 로그인이 성공(user가 false가 아닌 경우)
